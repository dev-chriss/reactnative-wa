...
const data = [
  ...
  Date.parse(a.registered) > Date.parse(b.registered) ? -1 : 1,
);
        ...
        <View style={routeStyles.callDetail}>
          {item.isGet ? (
            <AntDesign
              name="arrowdown"
              style={[
                routeStyles.getCallDetailIcon,
                item.isActive ? {color: '#25d065'} : {color: '#f06476'},
              ]}
            />
          ) : (
            <AntDesign
              name="arrowdown"
              style={[
                routeStyles.setCallDetailIcon,
                item.isActive ? {color: '#25d065'} : {color: '#f06476'},
              ]}
            />
          )}
          <Text style={routeStyles.storiesDate}>
...
const GetAllCall = ({callData}) => {
  return (
    <View>
      ...
      {callData?.length ? (
        callData.map(item => <GetCall item={item} key={item._id} />)
      ) : (
